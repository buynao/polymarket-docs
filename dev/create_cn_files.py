...
我们创建一个通用的翻译内容文件，然后为每个文件创建带有翻译标题的版本
"""
import os
import glob
...
for i, filepath in enumerate(files[:20], 1):
    ...
    if create_cn_file(filepath):
        success_count += 1
        print(f"✓ {i:3d}. {filename} -> {filename.replace('.md', '_CN.md')}")
    else:
        print(f"✗ {i:3d}. {filename} (失败)")

print(f"\n完成！成功创建 {success_count} 个中文文件")
